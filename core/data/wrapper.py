...
class GarmentDatasetWrapper(object):
    ...
    def new_loaders(self, batch_size=None, shuffle_train=True, multiprocess=False):
        ...
        if self.batch_size is None:
            raise RuntimeError('GarmentDatasetWrapper:Error:cannot create loaders: batch_size is not set')
        ...
        if self.validation is not None and self.test is not None:    
            ...
            self.loaders.validation = DataLoader(self.validation, self.batch_size, collate_fn=collate_fn, num_workers=4)
            self.loaders.test = DataLoader(self.test, self.batch_size, collate_fn=collate_fn, num_workers=4)
    ...
    def load_split(self, split_info=None, batch_size=None):
        ...
        if 'filename' in self.split_info and self.split_info['filename'] is not None:
            print('GarmentDatasetWrapper::Loading data split from {}'.format(self.split_info['filename']))
            ...
            print('GarmentDatasetWrapper::Loading data split from split config: {}: valid per type {} / test per type {}'.format(
        ...
        print('GarmentDatasetWrapper::Dataset split: {} / {} / {}'.format(
    ...
    def get_data_lists(self, training, validation, testing, split_info=None):
        ...
        if 'filename' in split_info and  os.path.exists(split_info['filename']):
            ...
            # Create data_configs directory if it doesn't exist
            import os
            data_configs_dir = "./data_configs"
            os.makedirs(data_configs_dir, exist_ok=True)
            
            save_path = os.path.join(data_configs_dir, "data_split.json")
