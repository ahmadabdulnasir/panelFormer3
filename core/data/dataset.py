...
class GarmentDataset(Dataset):
    def __init__(self, root_dir, sim_root, start_config, gt_caching=False, feature_caching=False, in_transforms=[]):
        ...
        print("GarmentDataset::Info::Total valid datanames is {}".format(self.config['size']))

        self.gt_cached, self.gt_caching = {}, gt_caching
        self.feature_cached, self.feature_caching = {}, feature_caching
        if self.gt_caching:
            print('GarmentDataset::Info::Storing datapoints ground_truth info in memory')
        if self.feature_caching:
            print('GarmentDataset::Info::Storing datapoints feature info in memory')
        ...
        if self.config['panel_classification'] is not None:
            ...
            raise RuntimeError('GarmentDataset::Error::panel_classification not found')
    ...
    def standardize(self, training=None):
        ...
        """
        # print('GarmentDataset::Using data normalization for features & ground truth')
        if 'standardize' in self.config:
            ...
            raise ValueError('GarmentDataset::Error::Standardization cannot be applied: supply either stats in config or training set to use standardization')
        # print(stats)
    ...
    def update_config(self, in_config):
        """Define dataset configuration:
            ...
            * Control obtaining values for datapoints"""

        # Handle both old and new config formats
        if isinstance(in_config, dict) and 'dataset' in in_config:
            # New config format with nested structure
            dataset_config = in_config['dataset']
            
            # Extract max_pattern_len and max_panel_len from the dataset section
            max_pattern_len = dataset_config.get('max_pattern_len', None)
            max_panel_len = dataset_config.get('max_panel_len', None)
            max_num_stitches = dataset_config.get('max_num_stitches', None)
            
            # Extract panel_classification path
            panel_classification = dataset_config.get('panel_classification', None)
            
            # Extract obj_filetag
            obj_filetag = dataset_config.get('obj_filetag', '')
            
            # Update config with extracted values
            self.config.update({
                'max_pattern_len': max_pattern_len,
                'max_panel_len': max_panel_len,
                'max_num_stitches': max_num_stitches,
                'panel_classification': panel_classification,
                'obj_filetag': obj_filetag
            })
            
            # Copy standardization data if available
            if 'standardize' in dataset_config:
                self.config['standardize'] = dataset_config['standardize']
            
            # Update the rest of the dataset config
            self.config.update(dataset_config)
            
            pattern_size_initialized = (max_pattern_len is not None and 
                                       max_panel_len is not None and 
                                       max_num_stitches is not None)
        else:
        ...
        if split_on == "img":
            ...
            for idx in range(len(self.dataset_start_ids) - 1):
                ...
                if idx in train_folders:
                    ...
                    raise ValueError('GarmentDataset::Error::Run on data split with split_on==folder')
    ...
    def _read_pattern(self, gt_folder, folder_elements, spec_dict,
        ...
        if not spec_list:
            raise RuntimeError('GarmentDataset::Error::*specification.json not found for {}'.format(gt_folder))
        patterns = []
    ...
    def save_gt_batch_imgs(self, gt_batch, datanames, data_folders, save_to):
        ...
        for idx, (name, folder) in enumerate(zip(datanames, data_folders)):
            ...
            try: 
                ...
                print('GarmentDataset::Error::{} serializing skipped: {}'.format(name, e))
                continue
    ...
    def save_prediction_single(self, prediction, dataname="outside_dataset", save_to=None, return_stitches=False):
        ...
        try: 
            ...
            print('GarmentDataset::Error::{} serializing skipped: {}'.format(dataname, e))
    ...
    def save_prediction_batch(self, predictions, datanames, data_folders, 
        ...
        for idx, (name, folder) in enumerate(zip(datanames, data_folders)):
            ...
            try: 
                ...
                print('GarmentDataset::Error::{} serializing skipped: {}'.format(folder, e))
                continue
            final_file = pattern.name + '_predicted__pattern.png'
    ...
    def _pred_to_pattern(self, prediction, dataname, return_stitches=False):
        ...
        """
        # undo standardization (outside of generic conversion function due to custom std structure)
        # Handle different config structures
        if 'standardize' in self.config:
            gt_shifts = self.config['standardize']['gt_shift']
            gt_scales = self.config['standardize']['gt_scale']
        elif 'dataset' in self.config and 'standardize' in self.config['dataset']:
            gt_shifts = self.config['dataset']['standardize']['gt_shift']
            gt_scales = self.config['dataset']['standardize']['gt_scale']
        else:
            raise ValueError('GarmentDataset::Error::Standardization data not found in config')
        ...
        for key in gt_shifts:
            ...
            if key == 'outlines' and len(pred_numpy.shape) == 2: 
                # Get max_pattern_len and max_panel_len from the correct location in config
                max_pattern_len = None
                max_panel_len = None
                
                # Try to get values from different config locations
                if 'max_pattern_len' in self.config and 'max_panel_len' in self.config:
                    max_pattern_len = self.config['max_pattern_len']
                    max_panel_len = self.config['max_panel_len']
                elif 'dataset' in self.config and 'max_pattern_len' in self.config['dataset'] and 'max_panel_len' in self.config['dataset']:
                    max_pattern_len = self.config['dataset']['max_pattern_len']
                    max_panel_len = self.config['dataset']['max_panel_len']
                
                # Ensure we have valid values (not None)
                if max_pattern_len is None or max_panel_len is None:
                    print(f"Warning: Using default values for max_pattern_len and max_panel_len")
                    max_pattern_len = 23  # Default value
                    max_panel_len = 14   # Default value
                
                # Convert to integers if they're not already
                max_pattern_len = int(max_pattern_len)
                max_panel_len = int(max_panel_len)
                
                pred_numpy = pred_numpy.reshape(max_pattern_len, max_panel_len, 4)
        ...
        try: 
            ...
            print('GarmentDataset::Warning::{}: {}'.format(dataname, e))
            pass
...
if __name__ == '__main__':
    from data.wrapper import GarmentDatasetWrapper
    import pdb; pdb.set_trace()
    start_config = {"panel_classification": "./configs/data_configs/panel_classes_condenced.json",
    ...
    dataset = GarmentDataset("DATASET/train", None, start_config)
    example_data = dataset[0]
    split_info = {"type": "percent", "split_on": "folder", "valid_per_type": 5, "test_per_type": 10}
    datawrapper = GarmentDatasetWrapper(dataset, batch_size=64)
