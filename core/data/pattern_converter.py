...
from utils.pattern.core import panel_spec_template
from utils.pattern.wrappers import VisPattern
from utils.pattern import rotation as rotation_tools
...
class NNSewingPattern(VisPattern):
    ...
    def multi_pattern_as_tensors(patterns, pad_panels_to_len=None, pad_panels_num=None, pad_stitches_num=None,
        ...
        for porder in panel_order:
            ...
            if len(cur_pat) == 0:
                ...
                raise RuntimeError(f'Multi_pattern_as_tensors() one pattern used more than once, {porder}')
        ...
        if stitches_num < stitches_num_org:
            raise ValueError(
                'Requested number of stitches {} is less the number of stitches {} in pattern {}'.format(
    ...
    def pattern_as_tensors(
        ...
        """
        if sys.version_info[0] < 3:
            raise RuntimeError('Pattern_as_tensors() is only supported for Python 3.6+ and Scipy 1.2+')
    ...
    def pattern_from_tensors(
        ...
        if sys.version_info[0] < 3:
            raise RuntimeError('Pattern_from_tensors() is only supported for Python 3.6+ and Scipy 1.2+')
        ...
        for idx in range(len(pattern_representation)):
            ...
            try:
                ...
                print("Warning Found Empty panel in the input")
                pass
        ...
        if stitches is not None and len(stitches) > 0:
            if not padded:
                ...
                raise NotImplementedError('Recovering stitches for unpadded pattern is not supported')
            ...
            print(f'Warning {self.name} Panels were updated but new stitches info was not provided. Stitches are removed.')
    ...
    def panel_as_numeric(self, panel_name, pad_to_len=None):
        ...
        """
        if sys.version_info[0] < 3:
            raise RuntimeError('Panel_as_numeric() is only supported for Python 3.6+ and Scipy 1.2+')
        ...
        if pad_to_len is not None:
            if len(edge_sequence) > pad_to_len:
                raise ValueError(f'Panel {panel_name} cannot fit into requested length: {len(edge_sequence)} edges to fit into {pad_to_len}')
    ...
    def panel_from_numeric(self, panel_name, edge_sequence, rotation=None, translation=None, padded=False):
        ...
        if padded:
            ...
            if len(edge_sequence) < 3:
                ...
                raise ValueError(f'Error: Supplied {panel_name}  is empty')
        ...
        if all(np.isclose(fin_vert, 0, atol=3)):  # 3 cm per coordinate is a tolerable error
            ...
            print(f'Warning: {self.name} with panel {panel_name}. Edge sequence do not return to origin. Creating extra vertex')
